...
LOCAL_PORT = 50051  # Local gRPC server port
TARGET_IP = "localhost"  # Target peer's IP address
TARGET_PORT = 50050  # Target peer's gRPC port
FLASK_PORT = 5001  # Port for the Flask web server
...
class ChatService(chat_pb2_grpc.ChatServiceServicer):
    ...
    def Chat(self, request_iterator, context):
        for message in request_iterator: # Loop over incoming messages from the request stream
            ...
            MESSAGE_QUEUE.put(message.message) # Put the received message in the message queue for retrieval by Flask
            yield message
...
def send_message_to_peer(message):
    with grpc.insecure_channel(f'{TARGET_IP}:{TARGET_PORT}') as channel: # Create a gRPC channel to communicate with the target peer
        ...
        response = stub.Chat(iter([chat_pb2.ChatMessage(message=message)])) # Send the message to the target peer using the Chat method and stream the response
...
def send_message():
    ...
    if message:
        reply = send_message_to_peer(message) # Send the message to the peer via gRPC
        return jsonify({'reply': reply}) # Return the peer's reply as a JSON response
